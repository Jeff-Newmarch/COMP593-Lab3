...
def process_sales_data(sales_csv, orders_dir):
    ...
    for order_id, order_df in sales_df.groupby('ORDER ID'):
        ...
        export_order_to_excel(order_df, order_id, orders_dir)

        

def export_order_to_excel(order_df, order_id, orders_dir):
    ...
    excel_writer = pd.ExcelWriter(order_path, engine='xlsxwriter')
    order_df.to_excel(excel_writer, index=False, sheet_name=sheet_name)
    ...
    worksheet.set_column(3, 3, 15, None)
